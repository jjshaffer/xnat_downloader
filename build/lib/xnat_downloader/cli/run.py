...
def parse_cmdline():
    ...
    #Include a single subject name as a command-line argument - jjs 4/30/2019
    parser.add_argument('-n', '--name', help='Provide subject id for renaming as command line argument', default=False)
...
class Subject:
    ...
    def __init__(self, proj_obj, label):
        ...
        #Add label value for subject ID to allow for use in storing DICOM images -jjs 5/1/2019
        self.label = label
        #print(self.label)
    ...
    def download_scan_unformatted(self, scan, dest, scan_repl_dict, bids_num_len,
        ...
        if scan not in self.scan_dict.keys():
            ...
            print scan, " is not in list of scans. Skipping..."
            return 0
        ...
        if scan not in scan_repl_dict.keys():
            ...
            #print scan, " is not in dictionary. Skipping..."
            return 0
        ...
        test = os.path.join(dcm_outdir,self.label)
        dcm_outdir=test
        #print(test)
        ...
        if potential_files:
            ...
            for rtry in range(max_retries):
                ...
                try:
                 ...
                 scan_par.scans().download(dest_dir=dcm_outdir, type=scan_id, name=scan_fmt, extract=True, removeZip=True)
    ...
    def download_scan(self, scan, dest, sub_label_prefix=None):
        ...
        if scan not in self.scan_dict.keys():
            print scan
            print scan, " is not available for download."
            #print('{scan} is not available for download'.format(scan=scan))
            return 1
...
def main():
    ...
    #If subject replacement name passed through command line, use it instead, otherwise, use list from json file -jjs 5/1/2019
    if opts.name:
        sub_repl_dict[opts.subject] = opts.name
        ...
        if sub_class.ses:
            for session in sub_class.ses_dict.keys():
                ...
                for scan in sub_class.scan_dict.keys():
                    #print scan
                    #print dest
                    #print scan_repl_dict
                    #print bids_num_len
                    #print sub_repl_dict
                    #print sub_label_prefix
                    #print scan in scan_repl_dict
